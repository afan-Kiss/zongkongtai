...
function createTerminal(shell: string, args: string[], cwd: string, env: NodeJS.ProcessEnv): TermBackend {
  const pty = loadPty();
  if (pty) {
...
export class ProcessManager extends EventEmitter {
  ...
  async preflight(project: {
    ...
    if (!fs.existsSync(project.localPath)) return { ok: false, message: `本地路径不存在：${project.localPath}` };

    const start = resolveStartCommand(project);
    if (!start) return { ok: false, message: '没有登记启动命令，请在设置或云端配置 desktop/dev/start 命令' };

    const pkg = path.join(project.localPath, 'package.json');
    if (!fs.existsSync(pkg) && !start.command.includes('python') && !/\.exe|\.bat/i.test(start.command)) {
