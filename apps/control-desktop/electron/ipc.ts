...
export function registerIpcHandlers(getMainWindow: () => BrowserWindow | null) {
  ...
  ipcMain.handle('config:save', (_e, partial: Record<string, unknown>) => {
    ...
    if (partial.adminPassword && partial.adminPassword !== '******') next.adminPassword = String(partial.adminPassword);

    if (partial.scanRoot) next.scanRoot = String(partial.scanRoot);

    if (partial.agentToken && !String(partial.agentToken).includes('****')) next.agentToken = String(partial.agentToken);

    if (partial.serviceToken && !String(partial.serviceToken).includes('****')) next.serviceToken = String(partial.serviceToken);
  ...
  ipcMain.handle('cloud:ports', async () => {
    ...
    const [ports, local] = await Promise.all([cloudClient.portConflicts(), Promise.resolve(scanLocalPorts())]);
  ...
  ipcMain.handle('process:usage', async (_e, projectId: string) => processManager.getUsage(projectId));
