...
const PRIORITY = [4723, 4725, 4726, 4727, 4730, 4790, 4791, 6780, 7788, 7789, 7890, 9322, 9323, 11434, 3001, 5173, 80, 443];
...
export function PortsPage() {
  ...
  const rows = cloud
    .filter((p) => PRIORITY.includes(p.port) || p.conflictLevel === 'conflict' || p.conflictLevel === 'warning')
    .slice(0, 80);
      ...
      {legacy4791 && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <Button size="sm" variant="secondary" onClick={close4791} disabled={closing || !legacy4791.listening}>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((p) => {
              ...
              const variant = level === 'conflict' ? 'destructive' : level === 'warning' ? 'warning' : 'success';
              ...
              return (
                <tr key={p.id} className="border-t border-border/50">
                  ...
                  <td className="p-3 text-xs">{rt ? `${rt.processName || '?'} (${rt.pid})` : '—'}</td>
