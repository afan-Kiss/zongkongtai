          ...
          <tbody>
            {projects.map((p) => (
              <tr key={p.id} style={p.archived ? { opacity: 0.55 } : undefined}>
                ...
                <td>
                  {[...new Set((p.ports || []).map((x: any) => x.port))].slice(0, 8).join(', ') ||
                    '-'}
                </td>
                <td>
                  ...
                  {!p.archived && (
                    <>
                      <button
                        ...
                        onClick={() => action(p.id, api.startProject, '启动')}
                      >
                        启动
                      </button>
                      <button
                        className="btn btn-sm btn-secondary"
                        onClick={() => action(p.id, api.stopProject, '停止')}
                      >
                        停止
                      </button>
                      <button
                        className="btn btn-sm btn-secondary"
                        onClick={() => action(p.id, api.restartProject, '重启')}
                      >
                        重启
                      </button>
                      <button
                        className="btn btn-sm btn-secondary"
                        onClick={() => action(p.id, api.healthCheck, '检查')}
                      >
                        检查
                      </button>
                    </>
