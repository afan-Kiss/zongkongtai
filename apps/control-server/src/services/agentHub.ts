    ...
    ws.on('message', async (raw) => {
      try {
        ...
        if (msg.type === 'scan_result') {
          ...
          const stats = await importScanResults(msg.payload as AgentScanPayload);
          await prisma.agent.update({
            where: { id: agent.id },
            data: { lastSeenAt: new Date(), status: 'online', basePath: msg.payload?.basePath },
          });
          const durationMs = msg.payload?.scanDurationMs ?? Date.now() - started;
          await prisma.operationLog.create({
            data: {
              ...
              detailJson: JSON.stringify({
                ...
                message: `${agent.name} 上传了一次扫描结果，发现 ${stats.projectCount} 个项目、${stats.portCount} 个端口、${stats.conflictCount} 个冲突、${stats.warningCount} 个提醒。`,
              }),
            },
          });
          return;
